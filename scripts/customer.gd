...
func try_to_find_seat():
	var tables = get_node(seat_container_path)
...
#func move_to_line_position(index: int):
	#in_queue = true
	#queue_index = index
	#var target_pos = waiting_line_positions[index]
	#agent.target_position = target_pos  # they will now walk to the new position
#
